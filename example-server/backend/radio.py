...
class RadioThread(Thread):
    ...
    def process_message(self, data):
        ...
        if not (record['flags'] & RadioFlags.ENABLE_BMP180) or (record['status'] & RadioFlags.BAROMETER_ERR):
            record['bar_temp'] = None
            record['bar_pres_abs'] = None
            record['bar_pres_rel'] = None

        if not (record['flags'] & RadioFlags.ENABLE_HTU21) or (record['status'] & RadioFlags.HUM_ERR):
            record['hum_temp'] = None
            record['hum_hum'] = None

        if not (record['flags'] & RadioFlags.ENABLE_TSL2561) or (record['status'] & RadioFlags.LIGHT_ERR):
            record['lux'] = None
